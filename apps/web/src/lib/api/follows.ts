...
import type { FollowActionResponse, FollowConnectionsPage, PendingFollowRequestsPage } from "@/lib/api/types";
...
export function listFollowers(params: { limit?: number; cursor?: string } = {}) {
  const limit = params.limit ?? 20;
  const query = new URLSearchParams();
  query.set("limit", String(limit));
  if (params.cursor) query.set("cursor", params.cursor);
  return authedFetch<FollowConnectionsPage>(`/api/v1/follows/followers?${query.toString()}`);
}

export function listFollowing(params: { limit?: number; cursor?: string } = {}) {
  const limit = params.limit ?? 20;
  const query = new URLSearchParams();
  query.set("limit", String(limit));
  if (params.cursor) query.set("cursor", params.cursor);
  return authedFetch<FollowConnectionsPage>(`/api/v1/follows/following?${query.toString()}`);
}
