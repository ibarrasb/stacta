...
export type FeedFilter = "ALL" | "REVIEW_POSTED" | "SCENT_POSTED" | "COLLECTION_ITEM_ADDED" | "WISHLIST_ITEM_ADDED" | "USER_FOLLOWED_USER" | "REVIEW_REPOSTED";
...
export function listMyPostFeed(params?: { limit?: number; cursor?: string }) {
  const query = new URLSearchParams();
  query.set("limit", String(params?.limit ?? 20));
  if (params?.cursor) query.set("cursor", params.cursor);
  return authedFetch<FeedPage>(`/api/v1/feed/me/posts?${query.toString()}`);
}

export function listUserPostFeed(username: string, params?: { limit?: number; cursor?: string }) {
  const query = new URLSearchParams();
  query.set("limit", String(params?.limit ?? 20));
  if (params?.cursor) query.set("cursor", params.cursor);
  return authedFetch<FeedPage>(`/api/v1/feed/users/${encodeURIComponent(username)}/posts?${query.toString()}`);
}
