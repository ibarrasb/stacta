CREATE TABLE IF NOT EXISTS user_wishlist_item (
  ...
  CONSTRAINT uq_user_wishlist_item UNIQUE (user_id, fragrance_source, fragrance_external_id)
);

CREATE INDEX IF NOT EXISTS idx_user_wishlist_item_user_added
  ON user_wishlist_item(user_id, added_at DESC);
